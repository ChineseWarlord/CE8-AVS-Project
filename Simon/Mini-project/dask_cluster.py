...
# %%
def DaskVectorizedMandelbrot(c: np.ndarray, *params: List[float]) -> np.ndarray:
    ...
    Args:
        ...
        params (List[float]):
            List containing max iterations and threshold.
...
def runDaskMandelbrotLocal(chunks: List[int], workers:int, process:bool) -> Tuple[np.ndarray, list]:
    ...
    for chunk in chunks:
        ...
        img_mandel = img_mandel.map_blocks(DaskVectorizedMandelbrot, *paramss, dtype=complex)
...
def runDaskMandelbrotCluster(chunks: List[int], params:List[float],IP:str, workers:int, process:bool) -> Tuple[np.ndarray, list]:
    ...
    # Creating np arrays for width/height pixels
    min_real, max_real, min_imaginary, max_imaginary, width, height, max_iter, T = params
    Px = np.linspace(min_real, max_real, width)
    Py = np.linspace(min_imaginary, max_imaginary, height)
    c = Px[np.newaxis, :] + 1j * Py[:, np.newaxis]
    ...
    for chunk in chunks:
        img_mandel = da.from_array(c, chunks=(chunk, chunk))
        t = time.time()
        #img_mandel = DaskVectorizedMandelbrot(c, *paramss)
        img_mandel = img_mandel.map_blocks(DaskVectorizedMandelbrot, *paramss, dtype=complex)
        #future = client.submit(img_mandel.map_blocks, DaskVectorizedMandelbrot, *paramss, dtype=complex)
        #futures.append(future)
        #img_mandel = img_mandel.result()
        img_mandel = img_mandel.compute()
        t2 = time.time() - t
        times.append(t2)
        print("Chunk:", chunk)
...
def chunks_plot(datatypes, time_values, baseline):
    plt.figure(figsize=(12, 8))
    plt.subplot(2, 1, 1)
    plt.title(f"Performance Analysis\nDask Vectorized VS Vectorized", fontsize=16)
    plt.plot(datatypes, time_values, marker="o", color="b")
    plt.xlabel("Chunks")
    plt.ylabel("Execution Time [s]")
    plt.grid()
...
if __name__ == "__main__":
    ...
    #chunks = [50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200, 250, 300, 350, 400, 450, 500]
    chunks = [1000,1500,2000,2500,3000,3500,4000,4500,5000]
    p = [min_real, max_real, min_imaginary, max_imaginary, width, height, max_iter, T]
    #img_mandel, times = runDaskMandelbrotLocal(chunks, 6, True)
    img_mandel, times = runDaskMandelbrotCluster(chunks,p, "10.92.0.71:8786", 8, True)
    chunks = [str(chunk) for chunk in chunks]
    chunks_plot(chunks, times, baseline)
